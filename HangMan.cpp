...
public:
  ...
  void getMaxRepeatedWord(ofstream&);
  void PrintList(ofstream&);
private:
  TreeNode* root;
};

void getMax(TreeNode*& tree, int* count, string* word);
...
int main()
{
  ...
  list.getMaxRepeatedWord(outFile);
  outFile.close();
  inFile.close();
  return 0;
}
...
void ListType::getMaxRepeatedWord(ofstream& outFile)
{
  int count = 0;
  string word = "test";
  getMax(root, &count, &word);
  outFile << "Max Word: " << word << " " << count;
}
...
void getMax(TreeNode*& tree, int* count, string* word)
{
  if (tree != NULL)
  {
    if (tree->info.count >= *count) {
      *count = tree->info.count;
      *word = tree->info.word;

      getMax(tree->left, count, word);
      getMax(tree->right, count, word);

    }
  }
}

// string getMax(TreeNode*& tree, int* count, string* word)
// {
...
//   // string left = getMax(tree->left, count, string);
//   // string right = getMax(tree->right, count, string);
//   // if (left > right){
//   //   return left
//   // }
// }
